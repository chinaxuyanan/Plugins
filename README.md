...
所以要用远端的话，只能 clone 下来走方式一的本地路径。**如果以后想让别人（或你自己的其它机器）直接按版本依赖**，最小的改动是在仓库根目录加一个 `Package.swift`，把三个库收成三个 product——那样三个包就变成一个包、共用一套版本号，但外部就能写 `.package(url: ".../Plugins", from: "0.16.0")` 了。需要的时候说一声。
